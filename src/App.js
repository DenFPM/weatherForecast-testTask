...
import {
  ...
  setCitiesData,
  setSelectedCities,
} from "./redux/actions/setWeatherActions";
...
const getAverageNumber = (array, type) => {
  switch (type) {
    ...
    case "tempFeel":
      return (
        array.reduce((a, b) => a + b.main.feels_like - 273.15, 0) / array.length
      );
    case "wind":
...
function App() {
  ...
  useEffect(()=>{
    const selectedCitiesFromLocalStorage = JSON.parse(
      localStorage.getItem("selectedCities")
    );
    console.log(selectedCitiesFromLocalStorage);
    if (
      selectedCitiesFromLocalStorage !== null &&
      selectedCitiesFromLocalStorage.length
    ) {
  ...
  const handleAddCity = (city) => {
    localStorage.setItem(
      "selectedCities",
      JSON.stringify([...selectedCities, city])
    );
  ...
  const generateWeatherCardData = (data) => {
    const currentDayData = data.list.filter((weatherTimeLine) =>
      weatherTimeLine.dt_txt.includes(data.list[0].dt_txt.slice(0, 10))
    );
    return {
      ...
      weatherTimeLines: {
        labels: currentDayData.map(
          (weatherTimeLine) =>
            `${new Date(weatherTimeLine.dt_txt).getHours()}:${new Date(
              weatherTimeLine.dt_txt
            ).getMinutes()}0`
        ),
        values: currentDayData.map((weatherTimeLine) =>
          parseInt(weatherTimeLine.main.temp - 273.15)
        ),
      },
    };
  };
      ...
      if (
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(async (position) => {
            const { data } = await getWeatherByLoc(
  ...
  return (
    <div className="container">
      <Header
        ...
        onOpenExistCityAlert={onOpenExistCityAlert}
      />
      {isAlertExistCityActive && (
        <Alert style={{ maxWidth: 200 }} severity="error">
          City already added!
        </Alert>
      )}
      <WeatherCards />
